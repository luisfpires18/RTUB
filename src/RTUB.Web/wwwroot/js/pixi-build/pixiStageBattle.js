  ...
  class StageBattleScene {
    ...
    createConsumableBar() {
      ...
      for (let i = 0; i < shown.length; i++) {
        ...
        bg.fill({ color: 1973806, alpha: 0.9 });
        bg.stroke({ color: 4473958, width: 1.5 });
        btnContainer.addChild(bg);
        let iconSprite = null;
        ...
        const qtyText = new PIXI.Text({
          text: `${qty}`,
          style: { fontSize: 10, fill: 16777215, fontWeight: "bold", fontFamily: "Arial" }
        });
        ...
        const cdText = new PIXI.Text({
          text: "",
          style: { fontSize: 12, fill: 16746564, fontWeight: "bold" }
        });
        cdText.anchor.set(0.5);
        cdText.x = btnSize / 2;
        cdText.y = btnSize / 2;
    ...
    async requestUseConsumable(type) {
      ...
      try {
        const result = await this.dotNetRef.invokeMethodAsync("OnUseConsumable", type);
        if (!result) {
    ...
    async requestPlayerAutoAttack() {
      ...
      try {
        const result = await this.dotNetRef.invokeMethodAsync("OnPlayerAutoAttack");
    ...
    async requestEnemyAttack(enemyIndex) {
      ...
      try {
        const result = await this.dotNetRef.invokeMethodAsync("OnEnemyAttack", enemyIndex);
        if (result) this.processServerResult(result);
      } catch (e) {
    ...
    async requestPlayerSpell(attackId, cooldownSeconds) {
      ...
      try {
        ...
        const result = await this.dotNetRef.invokeMethodAsync("OnPlayerSpell", attackId);
        if (result) {
    ...
    async requestTickCooldowns() {
      if (!this.dotNetRef || this.battleFinished) return;
      try {
        const result = await this.dotNetRef.invokeMethodAsync("OnTickCooldowns");
        if (result) {
    ...
    async requestTickConsumableCooldowns() {
      if (!this.dotNetRef || this.battleFinished) return;
      try {
        const result = await this.dotNetRef.invokeMethodAsync("OnTickConsumableCooldowns");
        if (result) {
    ...
    update() {
      ...
      if (this.interactiveMode && this.isPlaying) {
        ...
        if (this._cooldownTickAccum >= 1e3) {
          this._cooldownTickAccum -= 1e3;
          this.requestTickCooldowns();
        }
        this._consumableTickAccum += delta;
        if (this._consumableTickAccum >= 1e3) {
          this._consumableTickAccum -= 1e3;
