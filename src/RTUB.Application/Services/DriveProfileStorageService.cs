...
/// <summary>
/// Implementation of profile storage service using iDrive e2 (S3-compatible)
/// </summary>
public class DriveProfileStorageService : IProfileStorageService, IDisposable
{
    private readonly IAmazonS3 _s3Client;
    private readonly string _bucketName;
    private readonly ILogger<DriveProfileStorageService> _logger;
    private readonly string _environment;
    private readonly int _urlExpirationMinutes = 60; // URL expires after 1 hour
    private const string ProfilePathPrefix = "images/profile/";
    private const int WebPQuality = 85; // High quality WebP (0-100, where 100 is best quality)
    private const int MaxImageWidth = 800; // Max width for profile images (suitable for avatars)
    private const int MaxImageHeight = 800; // Max height for profile images (suitable for avatars)

    public DriveProfileStorageService(IConfiguration configuration, ILogger<DriveProfileStorageService> logger)
    {
        _logger = logger;
        ...
        // Profile storage requires write access to upload/delete images
    ...
    public async Task<string> UploadImageAsync(IFormFile file, string username)
    {
        ...
        try
        {
            ...
            if (image.Width > MaxImageWidth || image.Height > MaxImageHeight)
            {
                ...
                _logger.LogInformation("Resized profile image from {OriginalWidth}x{OriginalHeight} to {NewWidth}x{NewHeight}", 
            ...
            _logger.LogInformation("Successfully uploaded profile image to S3 as WebP: {ObjectKey}, Size: {Size} bytes", 
                objectKey, fileSize);
        ...
        {
            _logger.LogError(ex, "S3 error uploading profile image for username: {Username}. ErrorCode: {ErrorCode}, Message: {Message}", 
                username, ex.ErrorCode, ex.Message);
            throw new InvalidOperationException($"Failed to upload profile image: {ex.Message}", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error uploading profile image for username: {Username}", username);
            throw new InvalidOperationException($"Failed to upload profile image: {ex.Message}", ex);
        }
    }
    ...
    public Task<string?> GetImageUrlAsync(string filename)
    {
        ...
        {
            _logger.LogError(ex, "S3 error generating profile image URL for filename: {Filename}. ErrorCode: {ErrorCode}, Message: {Message}", 
                filename, ex.ErrorCode, ex.Message);
            return Task.FromResult<string?>(null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error generating profile image URL for filename: {Filename}", filename);
            return Task.FromResult<string?>(null);
        }
    }
    ...
    public async Task<bool> DeleteImageAsync(string filename)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully deleted profile image from S3: {ObjectKey}", objectKey);
            
            return true;
        }
        catch (AmazonS3Exception ex)
        {
            _logger.LogError(ex, "S3 error deleting profile image: {Filename}. ErrorCode: {ErrorCode}, Message: {Message}", 
                filename, ex.ErrorCode, ex.Message);
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error deleting profile image: {Filename}", filename);
            return false;
        }
    }
    ...
    public async Task<bool> ImageExistsAsync(string filename)
    {
        ...
        {
            _logger.LogError(ex, "S3 error checking if profile image exists: {Filename}. ErrorCode: {ErrorCode}, Message: {Message}", 
                filename, ex.ErrorCode, ex.Message);
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error checking if profile image exists: {Filename}", filename);
            return false;
        }
    }
    ...
    {
        return $"{ProfilePathPrefix}{_environment}/{filename}";
    }
    ...
    private string GenerateFilename(string username, string contentType)
    {
        ...
        // Sanitize username to remove any special characters
