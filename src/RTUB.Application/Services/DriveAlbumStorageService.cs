...
/// <summary>
/// Implementation of album storage service using iDrive e2 (S3-compatible)
/// </summary>
public class DriveAlbumStorageService : IAlbumStorageService, IDisposable
{
    private readonly IAmazonS3 _s3Client;
    private readonly string _bucketName;
    private readonly ILogger<DriveAlbumStorageService> _logger;
    private readonly string _environment;
    private readonly int _urlExpirationMinutes = 60; // URL expires after 1 hour
    private const string AlbumPathPrefix = "images/albums/";
    private const int WebPQuality = 85; // High quality WebP (0-100, where 100 is best quality)
    private const int MaxImageWidth = 800; // Max width for album images
    private const int MaxImageHeight = 800; // Max height for album images

    public DriveAlbumStorageService(IConfiguration configuration, ILogger<DriveAlbumStorageService> logger)
    {
        _logger = logger;
        ...
        // Album storage requires write access to upload/delete images
    ...
    public async Task<string> UploadImageAsync(IFormFile file, int albumId)
    {
        ...
        try
        {
            ...
            if (image.Width > MaxImageWidth || image.Height > MaxImageHeight)
            {
                ...
                _logger.LogInformation("Resized album image from {OriginalWidth}x{OriginalHeight} to {NewWidth}x{NewHeight}", 
            ...
            _logger.LogInformation("Successfully uploaded album image to S3 as WebP: {ObjectKey}, Size: {Size} bytes", 
                objectKey, fileSize);
        ...
        {
            _logger.LogError(ex, "S3 error uploading album image for album ID: {AlbumId}. ErrorCode: {ErrorCode}, Message: {Message}", 
                albumId, ex.ErrorCode, ex.Message);
            throw new InvalidOperationException($"Failed to upload album image: {ex.Message}", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error uploading album image for album ID: {AlbumId}", albumId);
            throw new InvalidOperationException($"Failed to upload album image: {ex.Message}", ex);
        }
    }
    ...
    public Task<string?> GetImageUrlAsync(string filename)
    {
        ...
        {
            _logger.LogError(ex, "S3 error generating album image URL for filename: {Filename}. ErrorCode: {ErrorCode}, Message: {Message}", 
                filename, ex.ErrorCode, ex.Message);
            return Task.FromResult<string?>(null);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error generating album image URL for filename: {Filename}", filename);
            return Task.FromResult<string?>(null);
        }
    }
    ...
    public async Task<bool> DeleteImageAsync(string filename)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully deleted album image from S3: {ObjectKey}", objectKey);
            
            return true;
        }
        catch (AmazonS3Exception ex)
        {
            _logger.LogError(ex, "S3 error deleting album image: {Filename}. ErrorCode: {ErrorCode}, Message: {Message}", 
                filename, ex.ErrorCode, ex.Message);
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error deleting album image: {Filename}", filename);
            return false;
        }
    }
    ...
    public async Task<bool> ImageExistsAsync(string filename)
    {
        ...
        {
            _logger.LogError(ex, "S3 error checking if album image exists: {Filename}. ErrorCode: {ErrorCode}, Message: {Message}", 
                filename, ex.ErrorCode, ex.Message);
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error checking if album image exists: {Filename}", filename);
            return false;
        }
    }
    ...
    {
        return $"{AlbumPathPrefix}{_environment}/{filename}";
    }
