...
public class AlbumTests
{
    ...
    [Fact]
    public void Create_WithIsDraftTrue_CreatesDraftAlbum()
    {
        // Arrange
        var title = "Draft Album";
        var year = 2020;
        ...
        // Act
        var result = Album.Create(title, year, description, isPrivate: isPrivate, isExclusive: isExclusive, isDraft: isDraft);
    ...
    [Fact]
    public void Create_WithoutIsDraft_CreatesReleasedAlbum()
    {
        // Arrange
        var title = "Released Album";
        var year = 2020;
    ...
    [Fact]
    public void UpdateDetails_WithIsDraft_UpdatesDraftSetting()
    {
        // Arrange
        var album = Album.Create("Test Album", 2020);
        album.IsDraft.Should().BeFalse();

        // Act
        album.UpdateDetails("Updated Title", 2021, "Updated desc", isPrivate: false, isExclusive: false, isDraft: true);

        // Assert
        album.IsDraft.Should().BeTrue();
        album.Title.Should().Be("Updated Title");
    }

    [Fact]
    public void UpdateDetails_ChangingFromDraftToReleased_UpdatesCorrectly()
    {
        // Arrange
        var album = Album.Create("Draft Album", 2020, "Description", isPrivate: false, isExclusive: false, isDraft: true);
        album.IsDraft.Should().BeTrue();

        // Act
        album.UpdateDetails("Released Album", 2021, "Now released", isPrivate: false, isExclusive: false, isDraft: false);

        // Assert
        album.IsDraft.Should().BeFalse();
        album.Title.Should().Be("Released Album");
    }
}
